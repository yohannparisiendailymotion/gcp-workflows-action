...
const gcfCommandPattern = 'gcloud functions deploy @@name@@ --gen2 --project=dailymotion-ads-@@env@@ --region=@@region@@ --runtime=@@runtime@@ --source=./dist --trigger-http --entry-point=@@entry_point@@';
/**
...
async function run() {
    try {
        ...
        specsObj.functions.forEach(fn => {
            ...
            variables.push(`${fn.env_name}=https://${fn.region}-dailymotion-ads-${specsObj.env}.cloudfunctions.net/${fn.name}`);
            jobs.push(gcfJob);
        });
        const wfJob = {
